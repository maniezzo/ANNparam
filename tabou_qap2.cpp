...
void tabu_search(int n,                  /* problem size */
  ...
  *best_cost = current_cost;
  ...
  for (current_iteration = 1; current_iteration <= nr_iterations && *best_cost > opt; 
   ...
   {
    tcpu = double(clock() - t0)/CLOCKS_PER_SEC;
    if(tcpu > tmax) 
    {  cout << "Breaking ... tpu " << tcpu << " tmax " << tmax << endl;
       break;
    }
...
int main(int argc, char *argv[])
 {int n;                    /* problem size */
  ...
  file_name = argv[1];
  cout << "Data file name : " << file_name << endl;
  ...
  printf("Number of tabu search iterations: %d\n",nr_iterations);
  nr_resolutions = atoi(argv[3]);
  printf("Number of trials: %d\n",nr_resolutions);
  //scanf("%d",&nr_resolutions);
  
  int tabu_duration = atoi(argv[4])*n;      /* parameter 4 (< n^2/2) */
  int aspiration = n*n*atoi(argv[5]);       /* parameter 5 (> n^2/2)*/
  cout << "tabu_duration: " << tabu_duration << endl; 
  cout << "aspiration: " << aspiration << endl;       /* parameter 2 (> n^2/2)*/

  tmax = (double) atoi(argv[6]);
  cout << "tmax: " << tmax << endl;       /* parameter 2 (> n^2/2)*/
  t0 = clock();
  ...
  for(no_res = 1; no_res <= nr_resolutions; no_res++)
   {generate_random_solution(n, solution);
    ...
    tabu_search(n, a, b,                     /* problem data */
               ...
               tabu_duration, aspiration,                   /* parameters */
   ...
   printf("Min cost: %f, Average cost: %f, average dev: %f\n", 
          mincost, somme_sol/nr_resolutions, 100*(somme_sol/nr_resolutions - opt)/opt);
